...
export function ContractorInfoForm({ currentOrganization, submitProfile, handleNavigate }) {
  return (
      ...
      {({ handleSubmit, pristine, dirtySinceLastSubmit, valid, submitting, submitError, submitErrors = {} }) => {
        ...
        return (
          <form onSubmit={handleSubmit}>
            ...
            <Field name="inn">{({ input, meta }) => <InputNumber input={input} meta={meta} label="ИНН" fullWidth disabled />}</Field>
            <Field name="reasonCode">{({ input, meta }) => <InputNumber input={input} meta={meta} label="КПП" fullWidth disabled />}</Field>
            <Field name="address">{({ input, meta }) => <Input input={input} meta={meta} label="Юридический адрес" fullWidth />}</Field>
            ...
            <Typography variant="h6" mb={3} fontWeight={"bold"}>
              Сводная аналитика по организации
            </Typography>
